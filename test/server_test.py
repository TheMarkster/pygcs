...
from pygcs.event_bus import EventHost, events, consumer, intercept_print, broadcast, get_metastate
...
def main():
    ...
    with PrintInterceptor(intercept_print(GlobalSignals.LOG)): # Convert print messages to events
        ...
        server.add_processor(EventProcessor(name='server_event_processor'))

        if server.connect():
            @consumer(GlobalSignals.DISCONNECTED)
            def handle_disconnect():
                print("❌ Disconnect signal received, shutting down server.")
                server.stop()
